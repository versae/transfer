...
/*
 * Pixastic Lib - Binarization filter - v0.0.1
 * Copyright (c) 2010 Javier de la Rosa, versae@gmail.com, http://versae.es
 * License: [http://www.pixastic.com/lib/license.txt]
 */

Pixastic.Actions.binarization = {
    process : function(params) {
        // Negative values in threshold is to invert image
        var threshold = 128;
        if (typeof params.options.threshold != "undefined")
            threshold = parseFloat(params.options.threshold) || 128;
        if (Pixastic.Client.hasCanvasImageData()) {
            ...
            var x = data.length;
            do {
                if (dataCopy[x] > Math.abs(threshold)) {
                    data[x] = 255;
                } else {
                    data[x] = 0;
                }
